...
- `writing-assignments/` — the assignment/activity page shape: Purpose · Task · Criteria · Reflection (adapted from the Transparent Assignment Template, Winkelmes 2013), the internal qualities line, and the pipeline the page lands in (`schedule.html` as source of truth → HTML page → Canvas iframe → reconcile). Triggers when a decided assignment needs its student-facing page. Sits above the three L1 skills and calls them.
