...
/**
 * Build local assignment URLs map from config assignments with htmlFile field
 * This replaces hardcoded localAssignmentUrls maps with CSV-driven data
 * @param {Object} config - The course config object
 * @param {string} basePath - Base path prefix for HTML files (default: '')
 * @returns {Object} Map of assignment keys to local HTML URLs
 */
function buildLocalUrlsFromConfig(config, basePath = '') {
  const localUrls = {};

  for (const [key, assignment] of Object.entries(config.assignments)) {
    if (assignment.htmlFile) {
      localUrls[key] = basePath + assignment.htmlFile;
    }
  }
...
/**
 * Render weekly learning section using htmlFile from config
 * Automatically builds localAssignmentUrls from config.assignments[].htmlFile
 * @param {Object} config - The course config object
 * @param {number} weekNum - Current week number
 * @param {string} containerId - ID of the container element for this week's items
 * @param {string} lookingAheadContainerId - ID of container for Looking Ahead card
 * @param {string} basePath - Base path prefix for HTML files (default: '')
 */
function renderWeeklyLearningFromConfig(config, weekNum, containerId, lookingAheadContainerId, basePath = '') {
  const localAssignmentUrls = buildLocalUrlsFromConfig(config, basePath);
  renderWeeklyLearning(config, weekNum, containerId, lookingAheadContainerId, localAssignmentUrls);
}

/**
 * Initialize a home page with CSV-loaded config
 * Asynchronously loads assignments from CSV and merges with base config
 ...
 * @param {string} options.basePath - Base path for HTML files
 * @param {Function} options.onReady - Callback when initialization is complete
 */
async function initHomePageFromCSV(baseConfig, csvUrl, options = {}) {
  const basePath = options.basePath || '';
  const onReady = options.onReady || function() {};

  try {
    // Check if CSVLoader is available
    if (typeof CSVLoader === 'undefined') {
      console.warn('CSVLoader not loaded, falling back to base config');
      initHomePage(baseConfig);
      onReady(baseConfig);
      return;
    }

    // Merge CSV data with base config
    const config = await CSVLoader.mergeConfigWithCSV(baseConfig, csvUrl);
    ...
    // Render weekly learning with CSV-sourced htmlFile paths
    const currentWeek = typeof getEffectiveCurrentWeek === 'function'
      ? getEffectiveCurrentWeek()
      : getCurrentWeekFromConfig(config);

    if (currentWeek) {
      renderWeeklyLearningFromConfig(config, currentWeek, 'due-this-week', 'looking-ahead-container', basePath);
    }

    console.log('Home page initialized from CSV');
    onReady(config);
  } catch (error) {
    console.error('Failed to load CSV, falling back to base config:', error);
    initHomePage(baseConfig);
    onReady(baseConfig);
  }
}
...
/**
 * Initialize a sprint page with CSV-loaded config
 * @param {Object} baseConfig - Base config with weekDates, sprints, etc.
 * @param {string} csvUrl - URL to the assignments CSV file
 * @param {Object} options - Optional configuration
 * @param {Function} options.onReady - Callback when initialization is complete
 */
async function initSprintPageFromCSV(baseConfig, csvUrl, options = {}) {
  const onReady = options.onReady || function() {};

  try {
    if (typeof CSVLoader === 'undefined') {
      console.warn('CSVLoader not loaded, falling back to base config');
      initSprintPageWithLoom(baseConfig);
      onReady(baseConfig);
      return;
    }
    ...
    console.log('Sprint page initialized from CSV');
    onReady(config);
  } catch (error) {
    console.error('Failed to load CSV, falling back to base config:', error);
    initSprintPageWithLoom(baseConfig);
